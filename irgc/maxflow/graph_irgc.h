...
#ifndef __GRAPH_IRLS_H__
#define __GRAPH_IRLS_H__

#include <string.h>
#include "block_irgc.h"
...
template <typename captype, typename tcaptype, typename flowtype> class GraphIrgc
{
public:
	typedef enum
	{
	...
	GraphIrgc(long node_num_max, long edge_num_max, void (*err_function)(char *) = NULL);
	...
	flowtype maxflow(bool reuse_trees = false, BlockIrgc<node_id>* changed_list = NULL);
...
public:
	...
	long get_node_num() { return node_num; }
	long get_arc_num() { return (long)(arc_last - arcs); }
	...
	void get_rcap(arc* a, captype& r_cap, captype& rev_r_cap);
...
private:
	...
	struct arc
	{
		...
		node		*tail;		// node the arc originates from
		arc			*next;		// next arc with the same originating node
		arc			*prev;		// next arc with the same terminating node
		//arc			*sister;	// reverse arc

		captype		r_cap;		// residual capacity
		captype		rev_r_cap;		// residual capacity

		captype getCapacity(bool reverse) {
			if(reverse) return rev_r_cap;
			else return r_cap;
		}
		node* getHead(bool reverse) {
			if(reverse) return tail;
			else return head;
		}
		arc* getNext(bool reverse) {
			if(reverse) return prev;
			else return next;
		}	

		void setCapacity(bool reverse, captype cap) {
			if(reverse) rev_r_cap = cap;
			else r_cap = cap;
		}
	};
	...
	DBlockIrgc<nodeptr>		*nodeptr_block;
	...
	BlockIrgc<node_id>		*changed_list;
	...
	void augment(arc *middle_arc, bool reverse);
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline typename GraphIrgc<captype,tcaptype,flowtype>::node_id GraphIrgc<captype,tcaptype,flowtype>::add_node(long num)
{
	assert(num > 0);
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline void GraphIrgc<captype,tcaptype,flowtype>::add_tweights(node_id i, tcaptype cap_source, tcaptype cap_sink)
{
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline void GraphIrgc<captype,tcaptype,flowtype>::add_edge(node_id _i, node_id _j, captype cap, captype rev_cap)
{
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline typename GraphIrgc<captype,tcaptype,flowtype>::arc* GraphIrgc<captype,tcaptype,flowtype>::get_first_arc()
{
	return arcs;
}

template <typename captype, typename tcaptype, typename flowtype> 
	inline typename GraphIrgc<captype,tcaptype,flowtype>::arc* GraphIrgc<captype,tcaptype,flowtype>::get_next_arc(arc* a) 
{
	return a + 1; 
}

template <typename captype, typename tcaptype, typename flowtype> 
	inline void GraphIrgc<captype,tcaptype,flowtype>::get_arc_ends(arc* a, node_id& i, node_id& j)
{
	assert(a >= arcs && a < arc_last);
	//i = (node_id) (a->sister->head - nodes);
	i = (node_id) (a->tail - nodes);
	j = (node_id) (a->head - nodes);
}

template <typename captype, typename tcaptype, typename flowtype> 
	inline tcaptype GraphIrgc<captype,tcaptype,flowtype>::get_trcap(node_id i)
{
	assert(i>=0 && i<node_num);
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline void GraphIrgc<captype,tcaptype,flowtype>::get_rcap(arc* a, captype& r_cap, captype& rev_r_cap)
{
	assert(a >= arcs && a < arc_last);
	r_cap = a->r_cap;
	rev_r_cap = a->rev_r_cap;
}

template <typename captype, typename tcaptype, typename flowtype> 
	inline void GraphIrgc<captype,tcaptype,flowtype>::set_trcap(node_id i, tcaptype trcap)
{
	assert(i>=0 && i<node_num); 
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline void GraphIrgc<captype,tcaptype,flowtype>::set_rcap(arc* a, captype rcap)
{
	assert(a >= arcs && a < arc_last);
	a->r_cap = rcap;
}
...
template <typename captype, typename tcaptype, typename flowtype> 
	inline typename GraphIrgc<captype,tcaptype,flowtype>::termtype GraphIrgc<captype,tcaptype,flowtype>::what_segment(node_id i, termtype default_segm)
{
	if (nodes[i].parent)
	{
...
//template <typename captype, typename tcaptype, typename flowtype> 
//	inline void GraphIrgc<captype,tcaptype,flowtype>::mark_node(node_id _i)
//{
//	node* i = nodes + _i;
//	if (!i->next)
//	{
