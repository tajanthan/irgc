...
#ifndef __BLOCK_IRLS_H__
#define __BLOCK_IRLS_H__
...
template <class Type> class BlockIrgc
{
public:
	...
	BlockIrgc(int size, void (*err_function)(char *) = NULL) { first = last = NULL; block_size = size; error_function = err_function; }

	/* Destructor. Deallocates all items added so far */
	~BlockIrgc() { while (first) { block *next = first -> next; delete[] ((char*)first); first = next; } }
...
template <class Type> class DBlockIrgc
{
public:
	/* Constructor. Arguments are the block size and
	   (optionally) the pointer to the function which
	   will be called if allocation failed; the message
	   passed to this function is "Not enough memory!" */
	DBlockIrgc(int size, void (*err_function)(char *) = NULL) { first = NULL; first_free = NULL; block_size = size; error_function = err_function; }

	/* Destructor. Deallocates all items added so far */
	~DBlockIrgc() { while (first) { block *next = first -> next; delete[] ((char*)first); first = next; } }
