...
public:
	...
	Energy(long var_num_max, long edge_num_max, void (*err_function)(const char *) = NULL);
...
inline Energy<captype,tcaptype,flowtype>::Energy(long var_num_max, long edge_num_max, void (*err_function)(const char *)) : Graph<captype,tcaptype,flowtype>(var_num_max, edge_num_max, err_function)
{
	Econst = 0;
                              ...
                              Value C, Value D)
{
	...
	*/
	// AJ
	if(B+C < 0) {// needed as floating point arithmetic is not accurate
		//cout << endl << B << " + " << C << " = " << B+C << " < 0; (" << x << ", " << y << ")";
		B = (int)round(B);
		C = (int)round(C);
	}
